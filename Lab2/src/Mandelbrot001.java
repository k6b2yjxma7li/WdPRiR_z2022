...
public class Mandelbrot001 {
    ...
    public static void calcMandelbrotImage(int[] xResolution, int[] yResolution, double[] xLimits, double[] yLimits) {
        double dx = (xLimits[1]-xLimits[0])/(double)(xResolution[1]-xResolution[0]);
        double dy = (yLimits[1]-yLimits[0])/(double)(yResolution[1]-yResolution[0]);

        double cReal = xLimits[0];
        double cImag = yLimits[0];

        for (int xPxNo = xResolution[0]; xPxNo < xResolution[1]; xPxNo++) {
            // pixel index should be relative here, hence xPxNo-xResolution[0]
            cReal = xLimits[0] + (double)(xPxNo-xResolution[0])*dx;
            for (int yPxNo = yResolution[0]; yPxNo < yResolution[1]; yPxNo++) {
                cImag = yLimits[0] + (double)yPxNo*dy;
    ...
    public static double mandelbrotReruns(int resolution, int rerunsNumber) throws InterruptedException {
        ...
        for (int i = 0; i < rerunsNumber; i++) {
            ...
            // creating number of threads that correspond with number of processors
            Thread[] threads = new Thread[threadCount];
            // outsourcing parts of image to threads
    ...
    public static void main(String[] args) throws IOException, InterruptedException {
        ...
        BufferedWriter writer = new BufferedWriter(new FileWriter("WdPRiR_z2022/Lab2/src/timings-threads.csv", false));
        for (int dim : dims) {
